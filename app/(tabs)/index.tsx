...
export default function HomeScreen() {
  ...
  const [toast, setToast] = useState("Connected to memo");
  const palette = useMemo(() => ({ success: "#35D07F", info: "#4DA3FF", warning: "#FFB547" }), []);

  const action = (message: string) => {
    setToast(message);
    setTimeout(() => setToast("Connected to memo"), 2200);
  };
  ...
  return (
    <ScreenContainer className="px-5" containerClassName="bg-background">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={[styles.serverCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <View style={styles.serverTop}>
            ...
            <View style={styles.flex}><Text style={[styles.cardLabel, { color: colors.muted }]}>ACTIVE SERVER</Text><Text style={[styles.serverName, { color: colors.foreground }]}>memo</Text><Text style={[styles.serverMeta, { color: colors.muted }]}>root@66.29.145.99 · Ubuntu 24.04</Text></View>
