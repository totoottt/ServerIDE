    ...
    <ScreenContainer className="px-5" containerClassName="bg-background">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <Pressable onPress={() => { setDraft(server); setShowServerSheet(true); }} style={({ pressed }) => [styles.serverCard, { backgroundColor: colors.surface, borderColor: colors.border, opacity: pressed ? 0.72 : 1 }]}>
          ...
          <View style={styles.serverStats}><View><Text style={[styles.statValue, { color: colors.muted }]}>—</Text><Text style={[styles.statCaption, { color: colors.muted }]}>LATENCY</Text></View><View><Text style={[styles.statValue, { color: colors.error }]}>Offline</Text><Text style={[styles.statCaption, { color: colors.muted }]}>STATUS</Text></View><View><Text style={[styles.statValue, { color: colors.muted }]}>Not tested</Text><Text style={[styles.statCaption, { color: colors.muted }]}>LAST CHECK</Text></View></View>
        </Pressable>
        ...
        <View style={styles.actionRow}><Pressable onPress={() => { if (!activeServer) { action("Add a server first"); setDraft(server); setShowServerSheet(true); return; } router.push("/(tabs)/ssh"); }} style={({ pressed }) => [styles.action, { backgroundColor: "#4DA3FF", opacity: pressed ? 0.8 : 1 }]}><IconSymbol name="chevron.left.forwardslash.chevron.right" size={19} color="#07111B" /><Text style={styles.actionText}>Open SSH</Text></Pressable><Pressable onPress={() => { if (!activeServer) { action("Add a server first"); return; } router.push("/(tabs)/network"); }} style={({ pressed }) => [styles.action, { backgroundColor: colors.surface, borderColor: colors.border, opacity: pressed ? 0.7 : 1 }]}><IconSymbol name="paperplane.fill" size={19} color="#46D8E7" /><Text style={[styles.actionText, { color: colors.foreground }]}>Run scan</Text></Pressable></View>
        ...
        <Modal visible={showServerSheet} transparent animationType="slide" onRequestClose={() => setShowServerSheet(false)}>
          <View style={styles.modalBackdrop}>
            <View style={[styles.sheet, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              <Text style={[styles.sheetHint, { color: colors.muted }]}>Enter the connection details. The profile stays offline until a real SSH/SFTP connection succeeds.</Text>
