...
export default function ServersScreen() {
  ...
  const reset = () => {
    ...
    setPassword(""); setPrivateKey(""); setPassphrase(""); setHostFingerprint("");
  ...
  const openEdit = async (server: ServerProfile) => {
    ...
    setPassword(secret?.password ?? ""); setPrivateKey(secret?.privateKey ?? ""); setPassphrase(secret?.passphrase ?? ""); setHostFingerprint(secret?.hostFingerprint ?? "");
  ...
  const save = async () => {
    ...
    const hasSecret = password || privateKey || passphrase || hostFingerprint || (jumpEnabled && (jumpPassword || jumpPrivateKey));
    if (hasSecret) await saveServerSecret(saved.id, {
      password: password || undefined, privateKey: privateKey || undefined, passphrase: passphrase || undefined, hostFingerprint: hostFingerprint || undefined,
  ...
  const verifyHost = () => {
    ...
    hostKeyMutation.mutate(
      { credentials: { host: host.trim(), port: Number(port) || 22, username: username.trim(), portKnockSequence: portKnockSequence.trim() || undefined, jumpHost: jumpEnabled && jumpHost.trim() && jumpUsername.trim() ? { host: jumpHost.trim(), port: Number(jumpPort) || 22, username: jumpUsername.trim(), password: jumpPassword || undefined, privateKey: jumpPrivateKey || undefined, passphrase: jumpPassphrase || undefined } : undefined, ...(authMethod === "ssh-key" ? { privateKey, passphrase: passphrase || undefined } : { password }) } },
      {
...
{showAdvanced && <View style={styles.advancedBox}>
  ...
  {forceKeyboardInteractive && <Text style={[styles.hint, { color: colors.muted }]}>You'll be asked for your OTP/2FA code each time you connect.</Text>}
