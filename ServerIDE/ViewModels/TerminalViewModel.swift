...
final class TerminalViewModel: ObservableObject {
    ...
    @Published private(set) var connectionState: TerminalConnectionState = .connecting
    @Published private(set) var connectionAttempts = 0
    ...
    func connect(force: Bool = false) {
        guard force || !hasCheckedConnection else { return }
        hasCheckedConnection = true
        connectionAttempts += 1
        connectionState = .connecting
        errorMessage = nil
        output += "\nChecking Network → Auth → Shell…\n"
        Task {
            do {
                ...
                output += "Network ✓  Auth ✓  Shell ✓  Ready ✓\nWorking directory: \(currentDirectory)\n"
            } catch {
                let reason = Self.readableConnectionError(error)
                connectionState = .failed(reason)
                errorMessage = reason
                output += "Connection error: \(reason)\n"
                output += "Check the server host/IP, port, username, password, and SSH service, then tap Reconnect.\n"
            }
        }
    }
    ...
    func run() {
        ...
        guard connectionState.isReachable else {
            errorMessage = "The SSH session is not connected. Tap Reconnect before running a command."
            return
        }
        ...
        Task {
            do {
                ...
                output += "Error: \(reason)\n"
                errorMessage = reason
                if reason.lowercased().contains("connect") || reason.lowercased().contains("network") {
                    connectionState = .failed(reason)
                }
            }
            isRunning = false
    ...
    private static func readableConnectionError(_ error: Error) -> String {
        if let sshError = error as? SSHConnectionError {
            return sshError.localizedDescription
        }
        let text = error.localizedDescription.trimmingCharacters(in: .whitespacesAndNewlines)
        return text.isEmpty ? "SSH connection failed." : text
    }
}
