...
Read RELEASE_0.7.0.md for the current changes, verification and unfinished items. This release responds to the 18-page issue PDF: explicit Files plist, chunked conflict-checked editor saves with backups, IP lookup error handling, persistent terminal access and accurate tab counts, split file sections, line wrapping, app icon, local Documents commands, GitHub account/repository access and sampled metric charts. The source has not been compiled with the iOS SDK in this environment.
